...
public class PostagemController {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Postagem encontrada"),
			@ApiResponse(code = 400, message = "Postagem inexistente no sistema") })
	@GetMapping("/{id}")
	...
	public ResponseEntity<PostagemModel> alterar(@Valid @RequestBody PostagemModel postagem) {
		return repository.findById(postagem.getId()).map(resp -> {
			...
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Não existe uma postagem com esse id");
		});
	}
